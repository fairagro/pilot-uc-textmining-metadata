  ...
  OPTIONAL { 
    ...
    try:
        ...
        for result in results["results"]["bindings"]:
            ...
            soilDepth.append(labelEn.lower())
            labelDe = result.get("labelDe", {}).get("value", "")
            soilDepth.append(labelDe.lower())
    except Exception as e:
        print(f"An error occurred: {e}")
    return soilDepth


def save_soilDepth_list(soilDepth, filename=r"C:\Users\husain\pilot-uc-textmining-metadata\data\Bonares\output\ConceptsList\soilDepth_list.json"):
    print(soilDepth)
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(soilDepth, f, ensure_ascii=False, indent=4)
    print(f"soilDepth list saved to {filename}")

# After getting the species list
if __name__ == "__main__":
    soilDepth = keywords_from_agrovoc()
    save_soilDepth_list(soilDepth)
